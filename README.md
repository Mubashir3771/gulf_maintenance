...
The bulk of the implementation (DocTypes, controller, API, Workflow, scripts, fixtures) was
generated by an AI agent (Claude) working in the bench under my direction, and validated by a
24-check end-to-end test via `bench execute` before committing. AI was fast at scaffolding and good
at verifying field names against the *actually installed* v16. But it made real mistakes — some it
caught from tracebacks, several I caught on review and corrected:

**Things I caught and fixed on review (flow / app logic):**

1. **Over-permissive role permissions.** The AI's first cut of the DocType permissions left
   unspecified permission flags to default, so every role effectively got full access — a
   `Maintenance Technician` could create and delete `Equipment`. I caught this auditing the perms and
   had it corrected to the intended matrix (Technician read-only on Equipment, read/write on
   Maintenance Request, etc.).

2. **The forms were unusable — missing reads on linked masters.** The AI had only set permissions on
   this app's own doctypes, so no maintenance role could actually *select* a Customer, Item,
   Warehouse or Employee, and the office couldn't raise the invoice — the form was practically dead.
   I directed the fix: grant additive read access on those standard doctypes from `after_install`,
   **not** via Custom DocPerm fixtures — because the moment a standard doctype gets any Custom DocPerm,
   Frappe ignores its built-in perms and would have silently stripped Accounts/Sales users.

3. **"Desktop icons were removed" — wrong.** When I asked for a workspace plus an app/desktop icon,
   the AI claimed desktop-icon functionality had been removed from Frappe and only built a workspace.
   That's outdated — I corrected it to register the app on the `/apps` launcher via the
   `add_to_apps_screen` hook and attach the maintenance roles, which is the current mechanism.

**Things the AI hit and debugged from the traceback/source (mostly environment/setup):**

4. **`server_script_enabled` — site vs bench config.** The gate refused to run with
   `ServerScriptNotEnabled` even after `bench --site … set-config server_script_enabled 1`. Reading
   `frappe/utils/safe_exec.py` showed the flag is read from `get_common_site_config()` — it can only
   be set bench-wide. Fix: `bench set-config -g server_script_enabled 1`.

5. **Workflow wouldn't insert.** Creating the Workflow raised `LinkValidationError` because the
   `Workflow State` / `Workflow Action Master` masters didn't exist yet — this version validates those
   links before auto-creation. Fixed by creating the masters first.

6. **The site wasn't actually set up.** The first labour-Item creation failed — no Item Groups, UOMs,
   Company or warehouses (the ERPNext setup wizard had never been completed). The wizard was run
   programmatically (Pakistan / PKR) to get a chart of accounts, default Income/COGS/Stock accounts
   and a default warehouse, which the Update-Stock invoice needs.

The pattern throughout: AI moves fast but needs review — permission scoping and "is the form actually
usable by a real role" were things I had to verify myself rather than trust the green test run.
